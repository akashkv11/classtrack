...
import { parseInput, syllabusClassIdQuerySchema, validationErrorResponse } from "@/lib/validation";

export function parseRequiredClassIdQuery(request: NextRequest) {
  const classId = request.nextUrl.searchParams.get("class_id");
  const parsed = parseInput(syllabusClassIdQuerySchema, { class_id: classId ?? "" });
